...
test('mapper#getScenarioSwitchText', () => {
  ...
  expect(option.option1).toBe('FCnoCRA');
...
test('mapper#getSwitchOptions', () => {
  ...
  expect(JSON.stringify(options)).toBe(
    JSON.stringify(['IDC2', 'FCnoCRA', 'FCnoSFA', 'FCnoCRAnoSFA']),
  );
  ...
  expect(JSON.stringify(options)).toBe(
    JSON.stringify(['IDC2', 'FC', 'FCnoSFA', 'FCnoCRAnoSFA']),
  );
  ...
  options = mapper.getSwitchOptions(testState);
  expect(JSON.stringify(options)).toBe(
    JSON.stringify(['IDC2', 'FC', 'FCnoCRA', 'FCnoCRAnoSFA']),
  );
});
...
test('mapper#getScenariosShip', () => {
  ...
  expect(JSON.stringify(result)).toBe(
    '{"IDC2":"rgba(43,131,186,1)","FC":"rgba(171,221,164,1)","FCnoCRA":"rgba(255,255,191,1)","FCnoSFA":"rgba(253,174,97,1)","FCnoCRAnoSFA":"rgba(215,25,28,1)"}',
  );
});
