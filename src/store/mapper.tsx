...
const scenariosFriendlyNames = Object.values(scenarioNameMap) as string[];
...
const getScenarioData = (
  ...
  const data: any = {
    ...
    friendlyNames: scenariosFriendlyNames,
    labels,
  };
...
const getScenarioSwitchText = (state: State) => {
  const scenario1Label = scenariosFriendlyNames[1];
  const scenario2Label = scenariosFriendlyNames[2];
  const scenario3Label = scenariosFriendlyNames[3];
...
const getSwitchOptions = (state: State): string[] => {
  const values = scenariosFriendlyNamesPermutation.find(
    (values: string[]) => values[0] === state.currentScenario,
  );
  if (!values) {
    return [];
  }
