...
export function datasetsFactory(
  ...
  if (attributeToFix) {
    for (const scenarioToFix in attributeToFix) {
      ...
      console.log(attributeToFix, scenarioToFix, props);
...
export function chartDataFactory(
  ...
  if (state.compareScenarioEnabled) {
    ...
    if (state.currentScenario === scenario0Label) {
      ...
      if (state.compareWithScenario === scenario4Label) {
        compareArray = scenario4Data;
        newLabel = `${scenario4Label} - ${scenario0Label}`;
      }
    } else if (state.currentScenario === scenario1Label) {
      baseArray = scenario1Data;

      if (state.compareWithScenario === scenario0Label) {
    ...
    } else if (state.currentScenario === scenario3Label) {
      ...
      if (state.compareWithScenario === scenario0Label) {
        compareArray = scenario0Data;
        newLabel = `${scenario0Label} - ${scenario4Label}`;
      }
...
export function createChartsAllBiomes(state: State, t: any, view: View) {
  ...
  lines.forEach((line: any) => {
    ...
    } else if (state.currentScenario === scenario0Label) {
